...
description: Create a conceptual architecture — a high-level, annotated sitemap of a product's top-level sections (areas), the key pages in each, a one-line purpose for every page, and any global or cross-cutting features (persistent overlays, sitewide search). This is for defining a NEW or REDESIGNED architecture — the forward-looking target structure, not a record of what exists today. Renders natively into a Figma file using that file's own color variables and text styles (text-only; no icons). Works from a brief, from screens already in the Figma file, or from a live site URL. Trigger when the user wants a "conceptual architecture", a "sitemap" or "site map" for a new or redesigned product, to "map the sections and pages" of something being designed, or to visualize a proposed structure in Figma. To map an EXISTING site as it is today (current-state audit / page inventory), use site-audit. For the detailed layer — navigation, content hierarchy, URL patterns, and user flows — use information-architecture instead.
---
