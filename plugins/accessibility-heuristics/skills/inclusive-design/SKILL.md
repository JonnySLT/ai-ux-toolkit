...
description: Design accessibly and inclusively from the start — proactive guidance during design, not an after-the-fact audit. Covers designing for the range of human ability (vision, motor, hearing, cognitive, situational), inclusive patterns (touch targets, focus order, error recovery, plain language, reduced-motion, colour-independent meaning), and how to bake WCAG in early. Complements accessibility-check (which audits a finished design). Trigger when the user asks how to "design for accessibility", "make this inclusive", "accessible design patterns", "design for screen readers / low vision / motor / cognitive", or wants a11y considered before build rather than after.
---
...
Fold the common success criteria into design decisions rather than leaving them for audit: colour contrast & non-colour cues, focus visibility & order, target size, labels/names, error identification + suggestions, no keyboard traps, and content structure. Then hand the finished design to `accessibility-check` to verify.
...
3. **States & content** — accessible names, error recovery, plain-language copy (hand wording to `content-design`), reduced-motion behaviour.
