...
| "map the structure / sitemap" | `information-architecture` (detailed layer: nav, hierarchy, URL patterns, user flows) · `conceptual-architecture` (high-level annotated sitemap — areas, key pages, purpose — rendered into Figma with the file's DS) |
