...
**Built for a UX team.** It assumes you're not working in a vacuum — first-class support for the collaborative reality of product work: stakeholder and kickoff interviews, research synthesis the whole team can act on, and developer-handoff specs. Shaped for UX designers and researchers working side by side.
...
| Skill | Plugin | Use it to… |
|---|---|---|
| `site-audit` | site-architecture | Capture an **existing** website as a tabbed Figma document: an Overview, a Site Architecture tree, and a Page Inventory, all in the file's own tokens. Current-state only, with an optional visual Site Map diagram. **Requires the Figma MCP.** |
...
- **Adding a plugin:** follow the existing shape (`.claude-plugin/plugin.json` + `README.md` ending in its install line + `skills/<name>/SKILL.md`), register it in `.claude-plugin/marketplace.json`, and update every count in this README (hero, install block, *What's inside*, repo tree). Then two things `validate.mjs` can't check for you: add the skill to its phase table **and**, if it could be confused with an existing skill, add an overlap row to *Which skill for which task?* here and to the routing map in `plugins/prompt-builder/hooks/routing-context.md` — plus its manual mirror `templates/CLAUDE.md`. A skill missing from the overlap map is a skill Claude will silently pick over its neighbour.
- **Bump versions on any change — both of them.** `claude plugin update` compares the plugin's `version`, not its content: if you ship changed skills under an unchanged version, every existing install silently keeps the old files and `update` still reports "already at the latest version". So on any change to a plugin, bump that plugin's `version` in **both** `plugins/<name>/.claude-plugin/plugin.json` and its `.claude-plugin/marketplace.json` entry — they must match, and `validate.mjs` now fails if they don't. Removing or moving a skill out of a plugin is breaking: bump major. Then bump the catalog `version` in `.claude-plugin/marketplace.json` as well.
- **The Figma companion file stays in sync automatically.** The [AI UX Toolkit Figma file](https://www.figma.com/design/n4Hh5v0xiIdezIiiP0Hiu5/AI-UX-Toolkit) mirrors these READMEs — a weekly scheduled sync (`scripts/figma-toolkit-sync/`) regenerates its *What's inside* index and per-plugin pages from the repo, so editing a plugin README (or the What's inside tables) updates Figma with no manual design work. If a plugin's requirement chips change, update the `REQUIREMENTS` map in `scripts/figma-toolkit-sync/extract.mjs`. See [`scripts/figma-toolkit-sync/README.md`](scripts/figma-toolkit-sync/README.md).
